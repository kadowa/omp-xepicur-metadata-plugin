...
/**
 * @file plugins/metadata/epc10/filter/Epc10DescriptionXmlFilter.inc.php
 *
...
class Epc10DescriptionXmlFilter extends PersistableFilter {
	/**
	 * Constructor
	 * @param $filterGroup FilterGroup
	 */
	function Epc10DescriptionXmlFilter($filterGroup) {
		$this->setDisplayName('Epicur 1.0');
	...
	function getClassName() {
		return 'plugins.metadata.epc10.filter.Epc10DescriptionXmlFilter';
	}
	...
	function &_buildDocumentHierarchy(&$doc, &$root, &$description) {
		// Get the schema.
		$epc10Schema =& $description->getMetadataSchema();
		...
		foreach ($properties as $propertyName => $property) { /* @var $property MetadataProperty */
			if ($description->hasStatement($propertyName)) {
				...
				if ($translated) {
					...
					assert(is_a($epc10Schema, 'Epc10Schema'));
